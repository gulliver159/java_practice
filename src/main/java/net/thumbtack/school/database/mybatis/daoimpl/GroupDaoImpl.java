...
public class GroupDaoImpl extends DaoImplBase implements GroupDao {
    ...
    public Group update(Group group) {
        ...
        try (SqlSession sqlSession = getSession()) {
            try {
                ...
                LOGGER.info("Can't change Group {} {}", group, ex);
                sqlSession.rollback();
                throw ex;
            }
            sqlSession.commit();
    ...
    public List<Group> getAll() {
        ...
        try (SqlSession sqlSession = getSession()) {
            ...
            LOGGER.info("Can't get All Groups {0}", ex);
            throw ex;
        }
    }
    ...
    public Trainee moveTraineeToGroup(Group group, Trainee trainee) {
        LOGGER.debug("DAO move Trainee to Group: {} to {}", trainee, group);
        try (SqlSession sqlSession = getSession()) {
            try {
                getGroupMapper(sqlSession).moveTraineeToGroup(group, trainee);
            } catch (RuntimeException ex) {
                LOGGER.info("Can't move Trainee to Group: {} to {}, {}", trainee, group, ex);
                sqlSession.rollback();
                throw ex;
            }
            sqlSession.commit();
    ...
    public void addSubjectToGroup(Group group, Subject subject) {
        LOGGER.debug("DAO add Subject to Group: {} to {}", subject, group);
        try (SqlSession sqlSession = getSession()) {
            try {
                getGroupMapper(sqlSession).addSubjectToGroup(group, subject);
            } catch (RuntimeException ex) {
                LOGGER.info("Can't add Subject to Group: {} to {}, {}", subject, group, ex);
                sqlSession.rollback();
                throw ex;
            }
            sqlSession.commit();
