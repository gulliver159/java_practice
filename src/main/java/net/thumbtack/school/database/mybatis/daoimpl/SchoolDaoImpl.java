...
public class SchoolDaoImpl extends DaoImplBase implements SchoolDao {
    ...
    public List<School> getAllLazy() {
        LOGGER.debug("DAO get all Schools");
        try (SqlSession sqlSession = getSession()) {
    ...
    public void update(School school) {
        LOGGER.debug("DAO change School {}", school);
        try (SqlSession sqlSession = getSession()) {
            try {
                getSchoolMapper(sqlSession).update(school);
            } catch (RuntimeException ex) {
                LOGGER.info("Can't change School {} {}", school, ex);
                sqlSession.rollback();
                throw ex;
            }
            sqlSession.commit();
    ...
    public void deleteAll() {
        ...
        try (SqlSession sqlSession = getSession()) {
            try {
                ...
                LOGGER.info("Can't delete all Schools {0}", ex);
                sqlSession.rollback();
                throw ex;
            }
            sqlSession.commit();
